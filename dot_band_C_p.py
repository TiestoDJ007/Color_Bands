...
if __name__ == "__main__":
    ...
    for nb_p in range(3):
        ...
        if nb_p == 0:
            for n in range(bands.nb_bands):
                band_px = ax1.scatter(bands.distance,
                                      bands.bands[Spin.up][n] - vasprun.efermi,
                                      s=dot_size[nb_p][n], color='r',
                                      marker='.')
            ax1.set_title('{}'.format('Atom '+Plot_Atom+' px'+vasprun_file.strip('vasprun' + '.xml')))
            ax1.hlines(0, labels_position[0], labels_position[-1])
            plt.savefig('{}'.format(saving_dictory + saving_file + '_px.png'),
                        dpi=300)
            plt.show()
        elif nb_p == 1:
            for n in range(bands.nb_bands):
                band_py = ax1.scatter(bands.distance,
                                      bands.bands[Spin.up][n] - vasprun.efermi,
                                      s=dot_size[nb_p][n], color='b',
                                      marker='.')
            ax1.set_title('{}'.format('Atom '+Plot_Atom+' py'+vasprun_file.strip('vasprun' + '.xml')))
            ax1.hlines(0, labels_position[0], labels_position[-1])
            plt.savefig('{}'.format(saving_dictory + saving_file + '_py.png'),
                        dpi=300)
            plt.show()
        elif nb_p == 2:
            for n in range(bands.nb_bands):
                band_pz = ax1.scatter(bands.distance,
                                      bands.bands[Spin.up][n] - vasprun.efermi,
                                      s=dot_size[nb_p][n], color='g',
                                      marker='.')
            ax1.set_title('{}'.format('Atom '+Plot_Atom+' pz'+vasprun_file.strip('vasprun' + '.xml')))
            ax1.hlines(0, labels_position[0], labels_position[-1])
